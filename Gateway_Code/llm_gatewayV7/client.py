...
def _post_with_retry(url, body, timeout, max_retries=4, wait=20):
    for attempt in range(max_retries):
        r = httpx.post(url, json=body, timeout=timeout)
        if r.status_code in (502, 503) and attempt < max_retries - 1:
            print(f"[client] {r.status_code} — waiting {wait}s before retry {attempt+1}/{max_retries-1}")
            time.sleep(wait)
            continue
        r.raise_for_status()
        return r
...
class LLM:
    ...
    def chat(self, prompt: str = None, *,
        ...
        r = _post_with_retry(f"{self.base_url}/v1/chat", body, self.timeout)
        return r.json()
