...
-   (Highly recommended) Turn on Github slack notifications
...
-   Turning on GitHub email notifications and use filters to focus on your teams activity.
-   Using the GitHub notifier extension.
...
- `#changelog` — what's just shipped. Owned by the Wizard & Docs team. Updates constantly as PRs merge.
- `#coming-soon` — what's shipping soon. Owned by the Marketing team. Posts a daily digest.

Both channels are powered by agentic workflows that scan merged PRs and feature flag changes and summarize them into the relevant channel. PR authors can opt in or out manually via the *Publish to changelog?* and *Alert Sales and Marketing teams?* checkboxes on the `posthog/posthog` PR template, or via the `@posthog` Slack app. See how to publish changelog for more detail.
