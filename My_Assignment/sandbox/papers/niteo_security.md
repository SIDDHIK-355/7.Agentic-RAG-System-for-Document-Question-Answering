...
* Go through the UK's NCSC training, EdApp's Cyber Security, and ESET's Cybersecurity Awareness Training.
...
* Using 1Password to store OTP (one-time-password) tokens is convenient and safe.
...
Always use HTTPS, under all circumstances, if the service does not support this, do not use it! You can use the HTTPS-Everywhere browser extension to ensure HTTPS versions of websites are used.

## Sharing Customer Data

Under no circumstances should you send any customer data unencrypted (that includes Slack!). Sharing customer data such as lists of email addresses can be done using Signal so that no one else has access to these private details.
