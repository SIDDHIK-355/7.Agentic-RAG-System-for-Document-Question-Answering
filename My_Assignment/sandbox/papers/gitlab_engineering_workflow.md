...
For the workflow that applies to the wider community see the contributing guide.
...
In line with our values of short toes, making two-way-door decisions
...
| --- | --- | --- |
| Triage | 4 hours from the 2nd occurrence of a broken `master` incident creation until assignment | group labeled on the incident |
...
Note: Recurring incidents are negatively impacting master pipeline stability and development velocity. Any untriaged, recurring incident will be automatically escalated to `#development` following this timeline:
...
Recurring broken `master` incidents are automatically escalated to `#development` unless it is triaged within 4 hours.
...
1. Check discussions on the failure notification in the triage DRI's group Slack channel to see if anyone is investigating the incident you are looking at. See Triage broken master for information on who the triage DRI is.
1. If there is not a clear DRI or action to resolve, use the dev escalation process to solicit help in the broken `master` incident.

**Note:** Stable branch failures follow the same process as described here, but incidents are tracked in gitlab-org/release/tasks. See stable branches documentation for details.

#### Escalation on weekends and holidays

Master broken incidents must be manually escalated to `#development` if
necessary. Without a manual escalation, the service level objective can extend
to the next working day; that is, triage DRI is expected to triage the incident
on the next working day. Regardless of when the label was applied, we always
consider an incident to be in an `escalated` state as long as it has the ~"
escalation::escalated" label, until the incident is resolved.
...
1. Identification
   - Review non-resolved broken `master` incidents for the same failure. If the broken `master` is related to a test failure, search the spec file in the issue search to see if there's a known `failure::flaky-test` issue.
   - If this incident is **due to non-flaky reasons**, communicate in `#development`, `#backend`, and `#frontend` using the Slack Workflow.
      ...
      - Create a revert MR directly to save some time in case we need to revert down the line.
        - If you are reverting an MR that performs a database migration, you need to follow the Deployment blockers process to prevent the migration from proceeding to deploy and running on staging and production.
        - If the migration is executed in any environments, communicate to the release managers in `#releases` channel and discuss whether it's appropriate to create another migration to roll back the first migration or turn the migration into a no-op by following Disabling a data migration steps.
   - If you identified that `master` fails **for a flaky reason**, and it cannot be reliably reproduced (i.e. running the failing spec locally or retrying the failing job):
      - Fast Quarantine the failing test to restore pipeline stability if the flakiness is continuously causing master pipeline incidents.
   ...
   - Identify the merge request that introduced the failures. There are a few possible approaches to try:
      ...
      - Look at the project activity, and search for keywords in the recent merged events.
      - Look at the recent commits on master and search for keywords you might see in the failing job/specs (e.g. if you see a `geo` spec file is failing, specifically the `shard` spec, search for those keywords in the commit history).
        - You can filter with the `Merge branch` text to only see merge commits.
...
1. (Optional) Pre-resolution
   ...
   - If the failures occur only in `test-on-gdk` jobs, it's possible to stop those jobs from being added to new pipelines while the cause is being fixed. See the runbook for details.
...
2. To confirm flakiness, you can use the `@gitlab-bot retry_job <job_id>` or the `@gitlab-bot retry_pipeline <pipeline_id>` command to retry the failed job(s), even if you are not a project maintainer.
   ...
   - **Note**, The `retry_job` command can fail for the following reasons:
     ...
     - If there is no response to either of the `retry` commands, you are likely invoking them in non-supported projects. If you'd like to request for the commands to be added to your project, please make an issue and inform `#g_development_anallytics`. You are encouraged to self-serve the MR following this example and submit it for review for maximum efficiency.
...
1. Backport the broken `master` incident to the maintained stable branches. See stable branches
...
1. Create an issue for the Development Analytics group describing how the broken `master` incident could have been prevented in the Merge Request pipeline.
...
- Maintainers should start a new merged results pipeline (for canonical MRs)
  ...
  There's no need to rebase once `master` has been fixed since we use merged results pipelines.
...
1. In your post outline *why* the merge request is urgent.
...
1. Assess whether the merge request is urgent or not. If in doubt, ask the requestor for more
...
1. There is a corresponding non-resolved broken `master` incidents.
...
Failure in  happens in `master` and is being worked on in .
```
...
- Otherwise, you must:
  1. Unset the
    ...
    for the `gitlab-org/gitlab` project.
...
broken at times. When this happens, check What to do when the validation pipeline failed for more details.

## Stable branches

To guarantee the readiness of any GitLab release, it is fundamental that failures on stable branches are addressed with priority, similar to master branch failures.
It is the merge request author's responsibility to backport the following to the maintained stable branches:

- Backport any master broken incident that impacts the maintained versions.
- Backport flaky failure fixes, including quarantined specs.
- Backport catch-up corrective actions that are not bugs (spec fixes, documentation changes, performance improvements, etc).
...
you need to follow the Security Release process.
...
1. If you need to schedule something or prioritize it, apply the appropriate labels (see Scheduling issues).
...
1. If you are working on an issue that requires access to specific features, systems, or groups, open an access request to obtain access on staging and production for testing your changes after they are merged.
...
1. If an issue entails frontend and backend work, consider separating the frontend and backend code into separate MRs and merge them independently under feature flags. This will ensure frontend/backend engineers can work and deliver independently.
...
- Sentry is our error reporting tool
- log.gprd.gitlab.net has production logs
- prometheus.gitlab.com has alerts for the production team
...
- **Monday, 19 days before the milestone begins**:
  ...
  - Development engineering manager provides product manager with prioritization input for `~type::maintenance` issues per cross-functional prioritization
  - Quality provides product manager with prioritization input for `~type::bug` issues per cross-functional prioritization
...
- **Monday, 9 days after the milestone begins**:
  - The development lead for each stage/section coordinates a stage/section level review with the quad cross-functional dashboard review process. After the stages/section level reviews are complete, the VP of Development coordinates a summary review with the CTO, VP of Product, and VP of UX.
...
- **Friday, the day the milestone ends**:
  ...
  - Feature flags should be flipped from default off to default on after verification to be in the release. See feature flags.
  - Merging by the milestone code cut-off (Friday) **does not guarantee** that the feature will be in the release. See release timelines.
  - Individual release post entries merged for all relevant issues.
  - By end of the day, milestone `%x.y` is expired.
- **On or around Wednesday, the day before the release date**:
  - Group Retrospective issues are updated with shipped and missed deliverables and team-members are tagged in the discussion.
- **Wednesday, the day before the release date**:
  - Milestone Cleanup runs on the schedule at Milestone cleanup schedule.
...
- **On or around the Wednesday immediately following the release day**:
  - Product plans are updated to reflect previous and current releases, including category epics and direction pages.
- **On or around the second Monday following the release day**:
  - Non-critical security patches are released.
...
See auto deploy transition guidance for details.
...
- GitLab, schedule
- GitLab Runner, schedule
- GitLab Gitaly, schedule
- GitLab charts, schedule
- GitLab QA, schedule
- Omnibus GitLab, schedule (only moving milestone for now, not labelling)
...
1. triaged,
1. assigned a priority and severity, and
1. have a plan to remediate within the assigned SLA for all instances of severities S1 and S2, and
1. assigned a backlog label, `~"backlog::prospective"` if it is expected to be worked on in the next 18 months, `~"backlog::no-commitment"` otherwise.
...
Security is our top priority. Our Security Team is raising the bar on security every day to protect users' data and make GitLab a safe place for everyone to contribute. There are many lines of code, and Security Teams need to scale. That means shifting security left in the Software Development LifeCycle (SDLC). You can find more secure development help in the `#security_help` Slack channel.
...
- Not In Scope
  - An operational issue of GitLab.com or a self managed customer environment. This falls under the on-call process.
...
1. Person requesting Rapid Engineering Response raises the issue to their own manager and the subject matter domain engineering manager (or the delegation if OOO).
...
1. Adjust priority and severity or security severity and priority if necessary, and work collaboratively on the determined resolution.
...
A high impact issue has a direct measurable impact on GitLab.com service levels or error budgets.
...
The SaaS Health Dashboard is the primary focus of this process for the GitLab application. `infradev` issues also live in the following supported projects, all of which are monitored for SLO compliance by triage-ops automation:

- Production Engineering
- Gitaly
- Runway

Additional projects with open `infradev` issues are visible in the SaaS Health Dashboard.
...
1. Assess Severity and `Priority` and apply the corresponding label as appropriate.
1. Provide as much information as possible to assist development engineering troubleshooting.

#### Development

1. Development directors are responsible for triaging Infradev issues regularly by following the triage process below.
1. Development managers are encouraged to triage issues regularly as well.
...
1. Review the severity on the issue to validate appropriate prioritization.
1. Ensure that the issue clearly explains the problem, the (potential) impact on GitLab.com's availability, and ideally, clearly defines a proposed solution to the problem.
...
The state of `infradev` issues is shared at the Operational Excellence Meeting meeting.

### Historical metrics

Historical trends and distribution of `infradev` issues over time are available in the Tableau Infradev Dashboard.
...
1. **Clearly state the scope of the problem, and how it affects GitLab SaaS Platforms**. Examples could include:
   ...
   1. Saturation issues: the problem could leave to increased saturation, latency issues due to resource over-utilization. example
   1. Service-level degradation: the problem is causing our service-level monitoring to degrade, impacting the overall SLA of GitLab.com and potentially leaving to SLA violations. example
   1. Alerts not resulting in incidents: alerts that are triggered but do not result in actionable incidents for the Tier 1 EOC (SRE on call), yet require action by the team owning the service. Examples include noisy alerts, alerts that need tuning, alerts that signal a growing problem that is not yet an incident but may become one, or alerts that no longer need to exist. The team owning the service that is triggering the alert is responsible for determining the correct outcome and taking action (proactive action, tune, or remove). This reduces alerting noise and distraction for the Tier 1 EOC to only include alerts that represent production incidents while ensuring all alerts are prioritized and addressed appropriately.
   1. Problems which extend the time to diagnosis of incidents: for example, issues which degrade the observability of GitLab.com, swallow user-impacting errors or logs, etc. These could lead to incidents taking much longer to clear, and impacting availability. example
   1. Deficiencies in our public APIs which lead to customers compensating by generating substantially more traffic to get the required results. example
...
1. **Ensure a realistic severity is applied**: review the availability severity label guidelines and ensure that applied severity matches. Always ensure all issues have a severity, even if you are unsure. The severity of the infradev issue should reflect the ongoing risk if the issue goes unresolved — it is independent of the severity of the incident that generated it.
1. **If possible, include ownership labels** for more effective triage. The product categories can help determine the appropriate stage group to assign the issue to.
...
1. **Provide a method for validating that the original issue still exists**
   ...
   1. Ideally, provide a link to a Grafana query or an ELK query and clear instructions on how to interpret the results to determine whether the problem is still occurring. Check the "Verfication" section in this issue as an example of this.
