...
You have to create a PR which modifies all CloudSQL Proxy conigurations to point to new instance in base folder of the repository
...
```
kubectl delete pv 
kubectl delete pvc  -n prod
```

7. Merge PR from step 5 - this will invoke a deploy again, to recreate the PVC via a buildkite job.
...
Guidance for Managed Services Platform (MSP) incidents is available in Managed Services incident response.