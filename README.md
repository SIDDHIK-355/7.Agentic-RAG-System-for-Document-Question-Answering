...
```
Session_7/
├── My_Assignment/   ← the assignment codebase (all work happens here)
├── Sir_Code/        ← reference code provided by the instructor
│   ├── S7code/          reference copy of the agent
│   └── llm_gatewayV7/   the LLM gateway the agent depends on
├── My_Notes/        ← HTML notes and study material
└── my_corpus_backup/← 50-article AI corpus (markdown)
```
...
├── fetch_corpus.py    # One-time fetch of the 50-article AI corpus
│
├── sandbox/           # Agent's file workspace (sandboxed)
│   ├── papers/            # 5 research papers (attention, CoT, DPO, LoRA, ReAct)
│   └── gitlab_*.md        # GitLab handbook corpus
│
└── state/             # Persistent memory (survives restarts, gitignored)
...
**Configure** — copy the template and fill in your keys (never commit the real `.env`):
```bash
cp .env.example .env        # in Session_7/ (gateway + LLM keys)
cp My_Assignment/.env.example My_Assignment/.env   # Tavily key for the agent
```
